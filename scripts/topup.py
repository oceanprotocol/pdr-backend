...
if __name__ == "__main__":
    ...
    for name, value in addresses.items():
        ...
        print(
            f"{name}: OCEAN: {ocean_bal:.2f}, Native: {rose_bal:.2f}"
        )
        ...
        if minimum_ocean_bal>0 and ocean_bal<minimum_ocean_bal:
            ...
            ocean_token.transfer(value,config.web3_config.w3.to_wei(topup_ocean_bal,'ether'),config.web3_config.owner,False)
            time.sleep(20)
        if minimum_rose_bal>0 and rose_bal<minimum_rose_bal:
            print(f"\t Transfering {topup_rose_bal} ROSE to {value}...")
            rose.transfer(value,config.web3_config.w3.to_wei(topup_rose_bal,'ether'),config.web3_config.owner,False)
            time.sleep(20)
