...
predictoor1_address = getenv(
    "PREDICTOOR1_ADDRESS", "0x005c414442a892077bd2c1d62b1de2fc127e5b9b"
)
...
for contract in result["data"]["predictContracts"]:
    ...
    if with_preds / count > 0.9:
        print(
            f"\t For {contract['token']['name']} , we found {with_preds} predictions for {contract['token']['name']} from a total of {count} slots - OK"
        )
    else:
        print(
            f"\t For {contract['token']['name']} , we found {with_preds} predictions for {contract['token']['name']} from a total of {count} slots - FAILED"
        )
...
for contract in result["data"]["predictContracts"]:
    ...
    if with_preds / count > 0.9:
        print(
            f"\t For {contract['token']['name']} , we found {with_preds} truevals for {contract['token']['name']} from a total of {count} slots - OK"
        )
    else:
        print(
            f"\t For {contract['token']['name']} , we found {with_preds} truevals for {contract['token']['name']} from a total of {count} slots - FAILED"
        )
