...
def test_paused_contracts_are_filtered_out(
    ...
    contract_found_before = contract_address.lower() in [
        addr.lower() for addr in feeds_before
    ]
    ...
    # Step 6: Verify contract does NOT appear in feed contracts query
    feeds_after = query_feed_contracts(subgraph_url=subgraph_url)
    contract_found_after = contract_address.lower() in [
        addr.lower() for addr in feeds_after
    ]

    assert not contract_found_after, (
        f"Paused contract {contract_address} should NOT be in "
        "feed contracts query results"
    )

    # Step 7: Verify contract does NOT appear in pending slots query
    ...
    assert len(slots_with_paused_contract) == 0, (
        f"Paused contract {contract_address} should NOT have any slots "
        "in query results"
    )

    # Step 8: Verify paused contract is filtered from pending payouts
    # (include_paused=False)
    ...
    # If there were payouts for this contract before, they should be gone
    if contract_address.lower() in [addr.lower() for addr in payouts_before]:
        assert contract_address.lower() not in [
            addr.lower() for addr in payouts_without_paused
        ], (
            f"Paused contract {contract_address} should NOT be in "
            "pending payouts (include_paused=False)"
        )
    ...
    if contract_address.lower() in [addr.lower() for addr in payouts_before]:
        assert contract_address.lower() in [
            addr.lower() for addr in payouts_with_paused
        ], (
