...
from pdr_backend.binmodel.binmodel_prediction import (
    BinmodelPrediction,
    _do_trust_models,
    _models_in_conflict,
)
...
@enforce_types
def test_binmodel_prediction_case1_models_in_conflict():
...
def test_binmodel_prediction_case2_up_dominates():
    p = BinmodelPrediction(conf_thr=0.1, prob_UP=0.6, prob_DOWN=0.3)
    assert p.conf_thr == 0.1
    assert p.prob_UP == 0.6
    ...
    p = BinmodelPrediction(conf_thr=0.3, prob_UP=0.6, prob_DOWN=0.3)
    assert not p.pred_up


@enforce_types
def test_binmodel_prediction_case3_down_dominates():
    p = BinmodelPrediction(conf_thr=0.1, prob_UP=0.4, prob_DOWN=0.7)
    assert p.conf_thr == 0.1
    ...
    # setup like above, but now with higher conf thr, which it can't exceed
    p = BinmodelPrediction(conf_thr=0.5, prob_UP=0.4, prob_DOWN=0.7)
    assert not p.pred_down
