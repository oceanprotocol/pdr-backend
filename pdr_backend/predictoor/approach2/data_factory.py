...
def pairstr(coin: str, usdcoin: str) -> str:
    """Eg given 'BTC','TUSD', return 'BTC/TUSD'"""
...
class DataFactory:
    ...
    def _update_hist_csv_at_exch_and_pair(self, exchange_id, pair):
        ...
        while True:
            ...
            if len(uts) > 1:
                ...
                diffs_ok = mn == mx == get_ms_per_epoch()
                if not diffs_ok:
                    print(f"**WARNING: diffs not ok: mn={mn}, mx={mx}**")
                # assert mx == mn == get_ms_per_epoch()
            raw_tohlcv_data = [
            ...
            if len(raw_tohlcv_data) < 1000:
                # no more data, we're at newest time
                break

            # prep next iteration
            newest_ut = int(df.index.values[-1])
            st_ut = newest_ut + get_ms_per_epoch()
    ...
    def _calc_start_ut_maybe_delete(self, filename: str) -> int:
        ...
        if self.ss.st_timestamp >= file_ut0:
            ...
            return file_utN + get_ms_per_epoch()
