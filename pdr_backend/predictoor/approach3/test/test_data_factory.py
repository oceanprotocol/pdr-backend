...
def _test_update_csv(st_str:str, fin_str:str, tmpdir, n_uts):
    ...
    def _uts_in_range(st_ut, fin_ut):
        return [_calc_ut(st_ut, i) for i in range(100000) #assume <=100K epochs
                if _calc_ut(st_ut, i) <= fin_ut]

    def _uts_from_since(cur_ut, since, limit):
        return [_calc_ut(since, i) for i in range(limit)
    ...
    ss.exchs_dict['binance'] = exchange

    # setup: data_factory, filename
    data_factory = DataFactory(ss)
    filename = data_factory._hist_csv_filename('binance', 'ETH/USDT')
    ...
    # work 1: new csv
    data_factory._update_hist_csv_at_exch_and_pair('binance', 'ETH/USDT')
...
def test_create_xy__1exchange_1coin_1signal(tmpdir):
    ...
    target_cols = ['binance:ETH:high:t-4', 'binance:ETH:high:t-3', 'binance:ETH:high:t-2']
    assert found_cols == target_cols
    
    assert found_cols.index('binance:ETH:high:t-2') == var_with_prev
    assert x_df['binance:ETH:high:t-2'].tolist() == [9,8,7,6,5,4,3,2]
    ...
    assert found_cols.index('binance:ETH:high:t-2') == var_with_prev
    assert x_df['binance:ETH:high:t-2'].tolist() == [10,9,8,7,6,5,4,3]
...
def test_create_xy__2exchanges_2coins_2signals(tmpdir):
    ...
    found_cols = x_df.columns.tolist()
    target_cols = ['binance:BTC:high:t-4', 'binance:BTC:high:t-3', 'binance:BTC:high:t-2', 'binance:BTC:low:t-4', 'binance:BTC:low:t-3', 'binance:BTC:low:t-2', 'binance:ETH:high:t-4', 'binance:ETH:high:t-3', 'binance:ETH:high:t-2', 'binance:ETH:low:t-4', 'binance:ETH:low:t-3', 'binance:ETH:low:t-2', 'kraken:BTC:high:t-4', 'kraken:BTC:high:t-3', 'kraken:BTC:high:t-2', 'kraken:BTC:low:t-4', 'kraken:BTC:low:t-3', 'kraken:BTC:low:t-2', 'kraken:ETH:high:t-4', 'kraken:ETH:high:t-3', 'kraken:ETH:high:t-2', 'kraken:ETH:low:t-4', 'kraken:ETH:low:t-3', 'kraken:ETH:low:t-2']
    assert found_cols == target_cols
    assert found_cols.index('binance:ETH:high:t-2') == var_with_prev

    # test binance:ETH:high like in 1-signal
    assert target_cols[6:9] == ['binance:ETH:high:t-4', 'binance:ETH:high:t-3', 'binance:ETH:high:t-2']
    Xa = X[:,6:9]
