...
class DataFactory:
    ...
    def _update_hist_csv_at_exch_and_pair(self, exchange_id, pair):
        ...
        while True:
            ...
            raw_tohlcv_data = [vec for vec in raw_tohlcv_data
    ...
    def create_xy(self, hist_df: pd.DataFrame, testshift:int):
        ...
        target_hist_cols = [f"{exchange_id}:{coin}:{signal}"
        ...
        for hist_col in target_hist_cols:
            ...
            if N_train <= 0:
                print(f"Too little data. len(z)={len(z)}, maxshift={maxshift}"
        ...
        x_col_with_prev = f"{ss.yval_exchange_id}:{ss.yval_coin}:" \
            f"{ss.yval_signal}:t-2"
    ...
    def _hist_csv_filename(self, exchange_id, pair) -> str:
        ...
        basename = exchange_id + '_' + pair.replace('/','-') + '_' + \
            self.ss.timeframe + '.csv'
