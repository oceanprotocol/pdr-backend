...
def test_base_config_pending_slots(monkeypatch):
    ...
    def _mock_get_pending_slots(arg0, timestamp, *args):
        return [f"1_{timestamp}", f"2_{timestamp}"]
    with patch("pdr_backend.models.base_config.get_pending_slots",
               _mock_get_pending_slots):
...
def test_base_config_feeds_contracts(monkeypatch):
    ...
    # test get_feeds()
    def _mock_query_predictContracts(*args, **kwargs):
        feeds_dict = {ADDR : "a mock_contract"}
        return feeds_dict
    with patch("pdr_backend.models.base_config.query_predictContracts",
    ...
    def _mock_contract(*args, **kwarg):
        m = Mock()
        m.contract_address = ADDR
        return m
    with patch("pdr_backend.models.base_config.PredictoorContract",
               _mock_contract):
