...
from pdr_backend.util.subgraph_predictions import get_all_predictions, FilterMode  # Adjust the import based on where you have saved the function
...
class TestGetAllPredictions(unittest.TestCase):
    ...
    def test_contract_filter_mode(self, mock_info_from_725, mock_get_subgraph_url, mock_query_subgraph):
        ...
        get_all_predictions(0, 10, ["contract1", "contract2"], "mainnet", FilterMode.CONTRACT)
        self.assertTrue(mock_query_subgraph.called, "Expected query_subgraph to be called.")
    ...
    @patch("pdr_backend.util.subgraph_predictions.get_subgraph_url")
    @patch("pdr_backend.util.subgraph_predictions.info_from_725")
    def test_predictoor_filter_mode(self, mock_info_from_725, mock_get_subgraph_url, mock_query_subgraph):
        mock_query_subgraph.return_value = {"data": {"predictPredictions": []}}
        mock_get_subgraph_url.return_value = "mock_url"
        mock_info_from_725.return_value = {
            "pair": "BTC/USD",
    ...
    def test_invalid_network(self, mock_get_subgraph_url, mock_query_subgraph):
        with self.assertRaises(Exception):
            get_all_predictions(0, 10, ["user1"], "invalid_network", FilterMode.PREDICTOOR)
