...
@enforce_types
def get_traction_info_main(ppss: PPSS):
    ...
    predictions_df = predictions_df.filter(
        (predictions_df["timestamp"] >= int(ppss.lake_ss.st_timestamp / 1000))
        & (predictions_df["timestamp"] <= int(ppss.lake_ss.fin_timestamp / 1000))
    )
