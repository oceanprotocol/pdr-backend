...
class ResidPlotdataFactory:

    @classmethod
    def build(cls, y_values, nlags: int) -> "ResidPlotdata":
...
@enforce_types
class ResidPlotdata:
    """Simple class to manage many inputs going to plot_resid."""
...
@enforce_types
def plot_resid(resid_plotdata: ResidPlotdata):
    """
    @description
      Plot resid & partial resid
    """
    d = resid_plotdata
    ...
    fig.update_yaxes(title_text="resid (acf)", row=1, col=1)

    # subplot 2: pacf
    _add_corr_traces(d.pacf_results, fig, row=2)
    fig.update_yaxes(title_text="partial resid (pacf)", row=2, col=1)
