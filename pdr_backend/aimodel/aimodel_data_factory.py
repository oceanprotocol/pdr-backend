...
class AimodelDataFactory:
    ...
    def create_xy(
        ...
        for hist_col in target_hist_cols:
            ...
            if N_train <= 0:
                s = "Too little data."
                s += "To fix: broaden time, or shrink testshift, max_diff, or autoregr_n"
                logger.error(s)
                sys.exit(1)
                
            for delayshift in range(ss.autoregressive_n, 0, -1):  # eg [4, 3, 2, 1, 0]
                ...
                x_list += [pd.Series(_slice(z_d0, -shift - N_train - 1, -shift))]
                xrecent_list += [pd.Series(_slice(z_d0, -shift, -shift + 1))]
                xcol_list += [x_col_d0]

                # diff=1
                if ss.max_diff >= 1: 
                    assert (shift + N_train + 1 + 1) <= len(z_d1)
                    x_col_d1 = hist_col + f":z(t-{delayshift+1})-z(t-{delayshift+1-1})"
                    x_list += [pd.Series(_slice(z_d1, -shift - N_train - 1, -shift))]
                    xrecent_list += [pd.Series(_slice(z_d1, -shift, -shift + 1))]
                    xcol_list += [x_col_d1]

                # diff=2
                if ss.max_diff >= 2: 
                    assert (shift + N_train + 1 + 1 + 1) <= len(z_d2)
                    x_col_d2 = hist_col + \
                        f":(z(t-{delayshift+1})-z(t-{delayshift+1-1})) - " \
                        f"(z(t-{delayshift+1-1})-z(t-{delayshift+1-1-1}))"
                    x_list += [pd.Series(_slice(z_d2, -shift - N_train - 1, -shift))]
