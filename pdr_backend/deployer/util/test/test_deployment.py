...
def test_build_image():
    with patch("pdr_backend.deployer.util.deployment.run_command") as mock_run_command:
        ...
        mock_run_command.assert_called_once_with("docker build -t image_name:image_tag .")

def test_push_image():
    with patch("pdr_backend.deployer.util.deployment.run_command") as mock_run_command:
        deployment.push_image("image_name", "image_tag", "registry_name")
        mock_run_command.assert_any_call("docker tag image_name:image_tag registry_name/image_name:image_tag")
        mock_run_command.assert_any_call("docker push registry_name/image_name:image_tag")

def test_deploy_agents_to_k8s():
    with patch("pdr_backend.deployer.util.deployment.run_command") as mock_run_command:
        deployment.deploy_agents_to_k8s("config_folder", "config_name")
        mock_run_command.assert_any_call("kubectl create namespace config-name") # sanitizes name _ => -
        mock_run_command.assert_any_call("kubectl apply -f config_folder/ -n config-name")
...
def test_destroy_cluster_pods(mock_shutil_which):
    ...
    with patch("pdr_backend.deployer.util.deployment.run_command") as mock_run_command:
        with patch("builtins.input", return_value="n"):
            deployment.destroy_cluster(mock_provider, "cluster_name", "config_name")
            mock_run_command.assert_called_once_with("kubectl delete pods --all -n config-name")
...
def test_cluster_logs(mock_shutil_which):
    ...
    mock_provider.cluster_exists.return_value = True
    with patch("pdr_backend.deployer.util.deployment.run_command") as mock_run_command:
        deployment.cluster_logs(mock_provider, "cluster_name", "app_name", "config_name")
        mock_run_command.assert_any_call("kubectl get pods -n config-name")
        mock_run_command.assert_any_call("kubectl logs -l app=app_name -f -n config-name")
...
def test_deploy_config(mock_shutil_which):
    ...
    with patch("pdr_backend.deployer.util.deployment.DeploymentInfo.read", return_value=mock_deployment_info):
        with patch("pdr_backend.deployer.util.deployment.deploy_cluster") as mock_deploy_cluster:
            with patch("pdr_backend.deployer.util.deployment.deploy_agents_to_k8s") as mock_deploy_agents_to_k8s:
                deployment.deploy_config("config_file", mock_provider)
                mock_deploy_cluster.assert_called_once_with(mock_provider, "config_name")
                mock_deploy_agents_to_k8s.assert_called_once_with("foldername", "config_name")
...
def test_destroy_config(mock_shutil_which):
    ...
    mock_deployment_info.config_name = "config_name"
    with patch("pdr_backend.deployer.util.deployment.DeploymentInfo.read", return_value=mock_deployment_info):
        with patch("pdr_backend.deployer.util.deployment.destroy_cluster") as mock_destroy_cluster:
            deployment.destroy_config("config_file", mock_provider)
            mock_destroy_cluster.assert_called_once_with(mock_provider, "config_name", "config_name")
...
def test_logs_config(mock_shutil_which):
    ...
    mock_deployment_info.config_name = "config_name"
    with patch("pdr_backend.deployer.util.deployment.DeploymentInfo.read", return_value=mock_deployment_info):
        with patch("pdr_backend.deployer.util.deployment.cluster_logs") as mock_cluster_logs:
            deployment.logs_config("config_file", mock_provider)
            mock_cluster_logs.assert_called_once_with(mock_provider, "config_name", "pdr-predictoor", "config_name")