...
class MEXCOrder(Order):
    ...
    @property
    def id(self): return self.order['id']
    @property
    def amount(self): return self.order['info']['origQty']
    @property
    def timestamp(self): return self.order['timestamp']
...
class Portfolio:
    ...
    def __init__(self, feeds: List[Feed]):
        self.sheets: Dict[str, Sheet] = {
            addr: Sheet(addr) for addr in feeds
        }
