...
def test_append_data(tmpdir):
    ...
    test_manager.append_data(example_df, dataset_identifier)
    ...
    # Since append_data doesn't immediately reflect in the filesystem, this test is limited


def test_finalize_data(tmpdir):
    test_manager = _get_test_manager(tmpdir)
    ...
    test_manager.append_data(example_df, dataset_identifier)
    test_manager.finalize_data(dataset_identifier)

    # Check if the files were created
    expected_path = os.path.join(
        str(tmpdir), dataset_identifier, "year=2022", "month=1", "1.parquet"
    )
