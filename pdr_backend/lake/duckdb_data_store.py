...
class DuckDBDataStore(BaseDataStore):
    ...
    @enforce_types
    def copy_table(
        self, source_table_name: str, target_table_name: str, with_data: bool = True
    ):
        """
        Copy the table from the source to the target.
        @arguments:
            source_table_name - The source table name.
            target_table_name - The target table name.
            with_data - A flag to copy the data.
        @example:
            copy_table("people", "people_copy", True)
        """
        
        if(with_data):
            self.execute_sql(f"CREATE TABLE {target_table_name} AS SELECT * FROM {source_table_name}")
            return
        
        self.execute_sql(f"CREATE TABLE {target_table_name} AS SELECT * FROM {source_table_name} WHERE 1=0")
    ...
    def related_update(
        ...
        data_source_column_name: str,
        data_target_column_name: str,
        source_match_column_name: str,
        target_match_column_name: str,
        target_table_ids_formula: str,
    ):
        """
        Update the target table based on the source table.
        @arguments:
            source_table_name - The source table name.
            target_table_name - The target table name.
            data_source_column_name - The source column name.
            data_target_column_name - The target column name.
            source_match_column_name - The source match column name.
            target_match_column_name - The target match column name.
            target_table_ids_formula - The formula for target table ids.
            @example:
            related_update(
                "people",
                "people",
                ...
                "id",
                "id",
                "(1, 2, 3)" # OR SPLIT_PART({}.ID, '-', 1) ||  '-' || SPLIT_PART({}.ID, '-', 2)
            
        self.execute_sql(
            f"""UPDATE {target_table_name}
            SET {data_target_column_name} = (
                SELECT {data_source_column_name}
                FROM {source_table_name}
                WHERE {source_match_column_name} = {target_match_column_name}
            )
            WHERE {target_match_column_name} IN {tuple(source_table_ids)}"""