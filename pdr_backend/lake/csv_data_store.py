...
class CSVDataStore:
    ...
    def _create_file_path(
        ...
        file_name = self._create_file_name(dataset_identifier, start_time, end_time, row_count)
    ...
    def write(self, dataset_identifier: str, data: pl.DataFrame):
        ...
        chunks = [
            data.slice(i, min(1000, len(data) - i))
            for i in range(0, len(data), 1000)]

        for i, chunk in enumerate(chunks):
            start_time = int(chunk['timestamp'][0])
            end_time = int(chunk['timestamp'][-1])
            file_path = self._create_file_path(dataset_identifier, start_time, end_time, len(chunk))
