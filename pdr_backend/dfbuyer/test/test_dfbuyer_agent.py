...
def test_new_agent(mock_get_address, mock_token, dfbuyer_config):
    ...
    assert call1 == call(
        dfbuyer_config.ppss.web3_pp.web3_config.w3.eth.chain_id, "PredictoorHelper"
    )
    call2 = mock_get_address.call_args_list[1]
    assert call2 == call(
        dfbuyer_config.ppss.web3_pp.web3_config.w3.eth.chain_id, "Ocean"
    )

    mock_token.assert_called_with(
        dfbuyer_config.ppss.web3_pp.web3_config, agent.token_addr
    )
...
def test_get_consume_so_far(mock_get_consume_so_far, dfbuyer_agent):
    agent = MagicMock()
    agent.config.ppss.web3_pp.web3_config.owner = "0x123"
