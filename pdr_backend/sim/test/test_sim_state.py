...
def test_hist_perfs__basic_init(dirn):
    ...
    assert hist_perfs.metrics_names() == target_names
    assert hist_perfs.metrics_names()[0] == f"acc_est_{dirn_s}"
    assert hist_perfs.metrics_names()[-1] == f"loss_{dirn_s}"
...
def test_hist_profits__basic_init():
    ...
    names = hist_profits.metrics_names()
    assert names == target_names
        
    values = hist_profits.recent_metrics_values()
    assert sorted(values.keys()) == sorted(names)
    for val in values.values():
        assert val is None
