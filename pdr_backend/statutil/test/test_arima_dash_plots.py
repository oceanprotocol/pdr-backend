...
def test_get_column_graphs():
    ...
    fig2 = Figure([])
    figure_data = [
        {"parent_id": "fig1_graph", "fig": fig1, "graph_id": "fig1_graph"},
        {"parent_id": "fig2_graph", "fig": fig2, "graph_id": "fig2_graph"},
    ]
    result = get_column_graphs("fig1_graph", figure_data, "title1", "")
    assert len(result) == 1
    assert len(result[0].children) == len(figure_data) + 2
    assert figure_data[0]["parent_id"] + "-title" == result[0].children[0].id
    assert figure_data[0]["parent_id"] == result[0].children[2].id
    assert figure_data[1]["parent_id"] == result[0].children[3].id


def test_display_on_column():
    div_id = "test_id"
    result = display_on_column(div_id)
